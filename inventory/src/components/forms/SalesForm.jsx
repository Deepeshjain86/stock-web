...
//   return (
//     <div className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold shadow-lg border mx-auto w-fit mb-4 ${isError ? 'bg-red-50 text-red-700 border-red-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
//       {isError ? <ExclamationCircleIcon className="w-4 h-4 flex-shrink-0" /> : <CheckCircleIcon className="w-4 h-4 flex-shrink-0" />}
//       {toast.msg}
//     </div>
//   );
// };

// const PAYMENT_METHODS = ['Cash', 'UPI', 'Card', 'Bank Transfer', 'Credit/Borrow'];
...
//         phone: u.phone || '',
//       };
//     } catch { return { name: 'Kirana Mart', gstin: '', address: '', phone: '' }; }
//   })();
...
//   const [customerType, setCustomerType] = useState('Walk-in'); // 'Walk-in' or 'Borrow'
...
//   /* ── Load Products, Customers & Warehouses from API ── */
//   const fetchProductsAndCustomers = async () => {
//     setLoadingProducts(true);
//     try {
//       const [prodRes, custRes, wareRes] = await Promise.all([
//         productsAPI.getAll({ limit: 500 }),
//         customersAPI.getAll().catch(() => ({ customers: [] })),
//         settingsAPI.getWarehouses().catch(() => ({ warehouses: [] }))
//       ]);
...
//       if (wareRes.success && wareRes.warehouses) {
//         setWarehousesList(wareRes.warehouses);
//         if (wareRes.warehouses.length > 0) {
//           setSelectedWarehouseId(String(wareRes.warehouses[0].id));
//         }
//       }
//     } catch (err) {
...
//   /* ── Calculations ── */
//   const subtotal = items.reduce((s, i) => s + (Number(i.price) * Math.max(1, Number(i.quantity) || 1)), 0);
...
//   const discountAmount = totalItemDiscount + (subtotal * (Number(globalDiscount) || 0) / 100);
...
//   const changeAmount = paymentMethod === 'Cash' && cashReceived ? Math.max(0, Number(cashReceived) - grandTotal) : 0;
...
//   /* ── Submit POS checkout ── */
//   const handleSubmit = () => {
//     if (items.length === 0) { showToast('Please add at least one product to the cart.'); return; }
//     if (paymentMethod === 'Cash' && cashReceived && Number(cashReceived) < grandTotal) {
//       showToast('Cash received is less than total amount.'); return;
//     }
//     if (customerType === 'Borrow' && (!customerId || customerId === 1)) {
//       showToast('You must select or register a Borrow Customer first.'); return;
//     }
//     if (paymentMethod === 'Credit/Borrow' && customerType !== 'Borrow') {
//       showToast('UDHAAR / Credit billing requires a registered Borrow Customer profile.'); return;
//     }
...
//       dueDate: paymentMethod === 'Credit/Borrow' ? dueDate : null,
//       amountPaid: paymentMethod === 'Credit/Borrow' ? Number(amountPaid) : 0,
//       paymentMethod,
//       paymentStatus: paymentMethod === 'Credit/Borrow' ? (Number(amountPaid) > 0 ? 'Partial' : 'Pending') : 'Paid',
//       date: billDate,
//       notes,
//       items,
//       subtotal,
...
//       warehouseId: Number(selectedWarehouseId || 1)
//     };
...
//   }, [items, customerType, customerId, paymentMethod, paymentStatus, cashReceived, amountPaid, dueDate, selectedWarehouseId]);
...
//   return (
//     <div className="flex flex-col h-[90vh] bg-slate-900 text-slate-100 font-sans select-none overflow-hidden">
      
//       {/* POS HEADER BAR */}
//       <div className="bg-slate-950 px-6 py-4 flex flex-col sm:flex-row items-center justify-between border-b border-slate-800 flex-shrink-0">
...
//           </div>
//           <div>
//             <h1 className="text-lg font-black tracking-tight text-white uppercase">{storeInfo.name} POS COUNTER</h1>
//             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Cashier Terminal: {user?.name || 'Staff'}</p>
//           </div>
//         </div>

//         <div className="flex items-center gap-3.5 mt-3 sm:mt-0">
//           <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs font-mono font-bold text-indigo-400">
//             <HashtagIcon className="w-4 h-4 text-indigo-500" />
//             {invoiceNumber}
//           </div>
//           <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs font-bold text-slate-300">
//             <CalendarDaysIcon className="w-4 h-4 text-indigo-500" />
//             <input
//               type="date"
//               value={billDate}
//               onChange={e => setBillDate(e.target.value)}
//               className="bg-transparent outline-none text-slate-100 cursor-pointer w-24 text-right"
//             />
//           </div>
//           <button
//             onClick={onCancel}
...
//       {/* POS 3-COLUMN SUPERMARKET VIEW LAYOUT */}
//       <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden bg-slate-900 p-4 gap-4" style={{ minHeight: 0 }}>
        
//         {/* COLUMN 1: CUSTOMER MANAGEMENT & PRODUCT SEARCH (LEFT - 3/12 COLS) */}
//         <div className="lg:col-span-3 flex flex-col bg-slate-950/80 border border-slate-800/80 rounded-2xl p-4 gap-4 overflow-y-auto">
//           <div>
//             <h3 className="text-xs font-black uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-2 flex items-center gap-2">
//               <UserIcon className="w-4 h-4 text-indigo-500" /> Customer Information
//             </h3>
//           </div>

//           {/* Segmented type switcher */}
//           <div>
//             <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Billing Category</label>
...
//                 }}
//                 className={`py-1.5 text-xs font-bold rounded-lg transition-all ${customerType === 'Walk-in' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
//               >
//                 Walk-in
//               </button>
//               <button
...
//                   const firstBorrow = customersList.find(c => c.customer_type === 'Borrow');
//                   if (firstBorrow) {
...
//                 }}
//                 className={`py-1.5 text-xs font-bold rounded-lg transition-all ${customerType === 'Borrow' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
//               >
//                 Borrow (Udhaar)
//               </button>
//             </div>
...
//           {/* Dropdown Customer Selector & Search */}
//           <div className="relative" ref={custDropdownRef}>
//             <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Select Profile</label>
//             <div className="flex gap-2">
//               <div
//                 onClick={() => setShowCustomerDropdown(!showCustomerDropdown)}
//                 className="flex-1 flex justify-between items-center bg-slate-900 border border-slate-800 rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-100 cursor-pointer"
//               >
...
//                 }}
//                 className="px-3 bg-indigo-650 hover:bg-indigo-700 text-white border border-indigo-600 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
...
//               <div className="absolute z-30 w-full bg-slate-950 border border-slate-800 rounded-xl shadow-xl mt-1 p-2 space-y-2 max-h-[220px] overflow-y-auto">
//                 <div className="flex items-center gap-2 border border-slate-850 rounded-lg px-2 py-1 bg-slate-900">
...
//                     className="w-full bg-transparent border-none text-xs text-white focus:outline-none py-1 font-semibold"
//                   />
//                 </div>
//                 <div className="space-y-1">
//                   {filteredCustomers.length === 0 ? (
//                     <div className="text-center text-[11px] text-slate-500 py-3 italic">No matching customers found</div>
//                   ) : (
...
//                           setSearchCustomer('');
//                         }}
//                         className="text-xs font-semibold text-slate-300 hover:bg-slate-900 hover:text-white p-2 rounded-lg cursor-pointer flex justify-between"
//                       >
//                         <span>{c.name}</span>
//                         <span className="text-[10px] text-slate-500 font-mono">{c.phone || 'No Mobile'}</span>
//                       </div>
//                     ))
//                   )}
//                 </div>
...
//             <div className="space-y-2 bg-slate-900/50 border border-slate-850 p-3 rounded-xl">
//               <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest block">Walk-in details</span>
//               <input
//                 type="text"
...
//                 className="w-full px-3 py-2 text-xs border border-slate-800 rounded-xl bg-slate-950 text-slate-200 font-semibold focus:outline-none focus:border-indigo-500"
//               />
//               <input
//                 type="text"
...
//                 className="w-full px-3 py-2 text-xs border border-slate-800 rounded-xl bg-slate-950 text-slate-200 font-semibold focus:outline-none focus:border-indigo-500 font-mono"
//               />
//             </div>
//           ) : (
...
//           {/* Warehouse Source Selector */}
//           <div>
//             <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Warehouse Source</label>
//             <select
//               value={selectedWarehouseId}
//               onChange={(e) => setSelectedWarehouseId(e.target.value)}
//               className="w-full px-3 py-2.5 text-xs border border-slate-800 rounded-xl bg-slate-900 text-slate-100 font-semibold focus:outline-none cursor-pointer"
//             >
//               {warehousesList.map(w => (
//                 <option key={w.id} value={w.id}>{w.name} ({w.location || 'Default'})</option>
//               ))}
//             </select>
//           </div>

//           {/* PRODUCT SEARCH SECTION */}
//           <div className="flex-1 flex flex-col justify-end" ref={dropdownRef}>
//             <h3 className="text-xs font-black uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-2 mb-2 flex items-center gap-2">
//               <QrCodeIcon className="w-4 h-4 text-indigo-550" /> Product Search & Barcode
//             </h3>
...
//                     if (!query) return;
//                     let match = productsList.find(p => String(p.barcode) === query || String(p.sku) === query);
//                     if (!match) {
//                       match = productsList.find(p => p.name?.toLowerCase() === query.toLowerCase());
//                     }
...
//                   }
//                 }}
//                 className="w-full pl-9 pr-4 py-2.5 text-xs border border-slate-800 rounded-xl bg-slate-900 text-slate-100 font-semibold focus:outline-none focus:border-indigo-500 placeholder:text-slate-500"
//               />
//             </div>

//             {/* Product Suggestions Dropdown */}
//             {showDropdown && (
//               <div className="mt-2 bg-slate-950 border border-slate-800 rounded-xl shadow-xl max-h-52 overflow-y-auto divide-y divide-slate-850 z-30 relative">
...
//                       <div
//                         key={p.id}
//                         onClick={() => inCart ? handleRemoveItem(items.findIndex(i => i.productId === p.id)) : handleAddProduct(p)}
//                         className={`flex items-center gap-3 px-4 py-2 cursor-pointer transition-colors ${inCart ? 'bg-indigo-900/40 hover:bg-indigo-900/50' : 'hover:bg-slate-900/60'}`}
//                       >
//                         <div className={`h-8 w-8 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-xs ${inCart ? 'bg-indigo-900 text-indigo-300' : 'bg-slate-900 text-slate-400'}`}>
//                           {(p.name || '?').charAt(0).toUpperCase()}
//                         </div>
//                         <div className="flex-1 min-w-0">
//                           <p className="text-xs font-semibold text-slate-200 truncate">{p.name}</p>
//                           <p className="text-[10px] text-slate-500 font-semibold">{p.barcode || '—'} • Stock: <span className={stock <= 0 ? 'text-rose-500' : 'text-emerald-500'}>{stock} {p.unit || 'Pcs'}</span></p>
//                         </div>
//                         <div className="text-right flex-shrink-0 text-xs">
//                           <p className="font-bold text-slate-100">₹{Number(p.selling_price || p.sellingPrice || 0).toFixed(2)}</p>
//                           {inCart ? (
//                             <span className="text-[9px] font-bold text-indigo-400 bg-indigo-950 px-1.5 py-0.5 rounded border border-indigo-900">✓ In Cart</span>
//                           ) : (
//                             <span className="text-[9px] font-bold text-emerald-400">+ Add</span>
//                           )}
//                         </div>
//                       </div>
//                     );
...
//         {/* COLUMN 2: BILLING CART (CENTER - 6/12 COLS) */}
//         <div className="lg:col-span-6 flex flex-col bg-slate-950/80 border border-slate-800/80 rounded-2xl overflow-hidden">
//           <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800 bg-slate-950 flex-shrink-0">
//             <div className="flex items-center gap-2">
//               <ShoppingCartIcon className="w-4.5 h-4.5 text-indigo-400 stroke-[2]" />
//               <span className="text-xs font-black uppercase tracking-wider text-slate-200">POS Billing Cart</span>
//               {items.length > 0 && (
//                 <span className="text-[10px] font-black text-indigo-300 bg-indigo-950/80 px-2 py-0.5 rounded-full border border-indigo-900">{items.length} Items</span>
//               )}
//             </div>
//             {items.length > 0 && (
//               <button
...
//                 type="button"
//                 className="text-[10px] font-bold text-rose-400 hover:text-rose-300 flex items-center gap-1 bg-slate-900 border border-slate-800 hover:bg-slate-800 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
//               >
//                 Clear Cart
//               </button>
//             )}
...
//                 <ShoppingCartIcon className="w-12 h-12 mb-3 stroke-1 text-slate-600" />
//                 <p className="text-sm font-bold text-slate-400">Cart is empty</p>
//                 <p className="text-xs text-slate-550 mt-1">Scan or search products to build the cashier receipt</p>
//               </div>
//             ) : (
//               <table className="w-full text-left border-collapse text-xs">
//                 <thead className="bg-slate-950/90 border-b border-slate-800 sticky top-0 z-10 text-[10px] font-bold uppercase tracking-wider text-slate-550">
//                   <tr>
//                     <th className="px-4 py-3 w-[40%]">Product</th>
//                     <th className="px-3 py-3 text-center w-28">Quantity</th>
...
//                     <th className="px-3 py-3 text-right w-28">Total Amount</th>
//                     <th className="px-2 py-3 w-10 text-center" />
//                   </tr>
//                 </thead>
//                 <tbody className="divide-y divide-slate-850 bg-slate-950/20 font-semibold text-slate-300">
//                   {items.map((item, idx) => (
//                     <tr key={idx} className="hover:bg-slate-900/40 transition-colors">
//                       <td className="px-4 py-3">
//                         <p className="text-xs font-bold text-slate-100 truncate max-w-[220px]">{item.name}</p>
//                         {item.barcode && <span className="text-[9px] font-mono text-slate-500">{item.barcode}</span>}
//                       </td>
//                       <td className="px-3 py-3">
//                         <div className="flex items-center justify-center gap-1.5">
//                           <button
//                             type="button"
//                             onClick={() => handleUpdateItem(idx, 'quantity', Math.max(1, Number(item.quantity) - 1))}
//                             className="h-5 w-5 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 flex items-center justify-center font-bold cursor-pointer"
//                           >
...
//                             className="w-10 text-center bg-slate-900 border border-slate-800 py-0.5 rounded text-xs font-bold text-white font-mono"
//                           />
//                           <button
//                             type="button"
//                             onClick={() => handleUpdateItem(idx, 'quantity', Number(item.quantity) + 1)}
//                             className="h-5 w-5 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 flex items-center justify-center font-bold cursor-pointer"
//                           >
...
//                           min="0"
//                           max="100"
//                           onChange={(e) => handleUpdateItem(idx, 'discount', Math.min(100, Math.max(0, Number(e.target.value) || 0)))}
//                           className="w-12 text-center bg-slate-900 border border-slate-800 py-0.5 rounded text-xs font-semibold text-slate-200 font-mono"
//                         />
//                       </td>
//                       <td className="px-3 py-3 text-center text-slate-400 font-mono">
//                         {item.gst}%
//                       </td>
//                       <td className="px-3 py-3 text-right font-bold font-mono text-white">
...
//                           className="text-rose-500 hover:text-rose-400 p-1 rounded hover:bg-slate-900 transition-all cursor-pointer"
//                         >
...
//         {/* COLUMN 3: BILL SUMMARY & CHECKOUT (RIGHT - 3/12 COLS) */}
//         <div className="lg:col-span-3 flex flex-col bg-slate-950/80 border border-slate-800/80 rounded-2xl p-4 gap-4 overflow-y-auto justify-between">
          
//           <div className="space-y-4">
//             <div>
//               <h3 className="text-xs font-black uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-2 flex items-center gap-2">
//                 <DocumentArrowDownIcon className="w-4 h-4 text-indigo-500" /> Tally Summary
//               </h3>
//             </div>

//             {/* Calculations metrics box */}
//             <div className="space-y-2 text-xs font-semibold text-slate-300">
//               <div className="flex justify-between bg-slate-900 border border-slate-850 p-2.5 rounded-xl">
//                 <span>Subtotal:</span>
//                 <span className="font-mono text-slate-200">₹{subtotal.toFixed(2)}</span>
//               </div>
//               <div className="flex justify-between bg-slate-900 border border-slate-850 p-2.5 rounded-xl">
//                 <span>Total item discount:</span>
//                 <span className="font-mono text-rose-400">-₹{totalItemDiscount.toFixed(2)}</span>
//               </div>
//               <div className="flex justify-between bg-slate-900 border border-slate-850 p-2.5 rounded-xl">
//                 <span>Tax (GST):</span>
//                 <span className="font-mono text-slate-200">₹{totalGst.toFixed(2)}</span>
//               </div>
              
//               <div className="bg-indigo-950 border border-indigo-900/60 rounded-xl p-3 flex justify-between items-center text-sm font-black text-indigo-300 shadow-inner">
//                 <span>Grand Total:</span>
//                 <span className="font-mono text-base text-white">₹{grandTotal.toFixed(2)}</span>
//               </div>
//             </div>

//             {/* Payment Section */}
//             <div>
//               <h3 className="text-xs font-black uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-2 mb-2.5 flex items-center gap-2">
//                 <CreditCardIcon className="w-4 h-4 text-indigo-500" /> Checkout Settlement
//               </h3>
              
//               <div className="grid grid-cols-2 gap-1.5 mb-3 p-1 bg-slate-900 border border-slate-850 rounded-xl">
//                 {PAYMENT_METHODS.map(mode => (
//                   <button
//                     key={mode}
//                     type="button"
//                     onClick={() => {
//                       if (mode === 'Credit/Borrow') {
//                         setCustomerType('Borrow');
//                       }
//                       setPaymentMethod(mode);
//                     }}
//                     className={`py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border transition-all cursor-pointer ${paymentMethod === mode ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-slate-950/20 border-transparent text-slate-400 hover:text-slate-200'}`}
//                   >
...
//               {/* Dynamic Cash Received parameters */}
//               {paymentMethod === 'Cash' && (
//                 <div className="space-y-2.5 bg-slate-900/50 border border-slate-850 p-3 rounded-xl">
//                   <div>
//                     <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Cash Received from customer</label>
//                     <input
//                       type="number"
//                       value={cashReceived}
//                       onChange={e => setCashReceived(e.target.value)}
//                       placeholder="e.g. ₹500"
//                       className="w-full px-3 py-2 text-xs border border-slate-800 rounded-xl bg-slate-950 text-slate-100 font-bold font-mono focus:outline-none focus:border-indigo-500"
//                     />
//                   </div>
//                   {Number(cashReceived) >= grandTotal && (
//                     <div className="flex justify-between items-center text-xs font-extrabold text-emerald-450">
...
//               {/* Dynamic Credit/Borrow parameters */}
//               {paymentMethod === 'Credit/Borrow' && (
//                 <div className="space-y-2.5 bg-slate-900/50 border border-slate-850 p-3 rounded-xl text-xs font-semibold text-slate-350">
//                   <div>
//                     <label className="text-[9px] font-bold text-slate-500 uppercase block mb-1">Payment Due Date</label>
//                     <input
//                       type="date"
//                       value={dueDate}
//                       onChange={e => setDueDate(e.target.value)}
//                       className="w-full px-3 py-1.5 border border-slate-800 rounded-xl bg-slate-950 text-slate-100 font-bold focus:outline-none cursor-pointer text-center"
//                     />
//                   </div>
//                   <div>
//                     <label className="text-[9px] font-bold text-slate-500 uppercase block mb-1">Down Payment (Optional)</label>
//                     <input
//                       type="number"
//                       value={amountPaid}
//                       onChange={e => setAmountPaid(e.target.value)}
//                       className="w-full px-3 py-1.5 border border-slate-800 rounded-xl bg-slate-950 text-slate-100 font-bold font-mono focus:outline-none text-right"
//                     />
//                   </div>
//                   <div className="flex justify-between border-t border-slate-800 pt-2 font-bold text-[10px]">
//                     <span>Credit Outstanding:</span>
//                     <span className="font-mono text-rose-400">₹{Math.max(0, grandTotal - Number(amountPaid)).toFixed(2)}</span>
//                   </div>
//                 </div>
//               )}
//             </div>
...
//               type="button"
//               className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-md shadow-emerald-600/10 active:scale-95 transition-all cursor-pointer"
//             >
//               <CheckCircleIcon className="w-4 h-4 stroke-[3.5]" /> Complete Sale (F8)
//             </button>
//             <button
//               onClick={onCancel}
//               type="button"
...
//       {/* INLINE POPUP MODAL: REGISTER NEW CUSTOMER */}
//       {showAddCustomerModal && (
//         <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center z-[99999]">
//           <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 space-y-4 shadow-2xl relative font-sans text-xs">
//             <button
...
//                 <UserPlusIcon className="w-5 h-5 text-indigo-500" /> Register Master Customer Profile
//               </h2>
//               <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Profile registers instantly in database ledger</p>
//             </div>

//             <form onSubmit={handleAddNewCustomerSubmit} className="space-y-3">
//               <div>
//                 <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Customer Type</label>
//                 <div className="grid grid-cols-2 gap-1 p-1 bg-slate-950 border border-slate-800 rounded-xl">
//                   <button
//                     type="button"
//                     onClick={() => setNewCustType('Walk-in')}
//                     className={`py-1 rounded-lg text-xs font-bold transition-all ${newCustType === 'Walk-in' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
//                   >
//                     Walk-in
//                   </button>
//                   <button
...
//                     className={`py-1 rounded-lg text-xs font-bold transition-all ${newCustType === 'Borrow' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
//                   >
//                     Borrow Customer
//                   </button>
//                 </div>
...
//                   className="w-full px-3 py-2 border border-slate-800 rounded-xl bg-slate-950 text-slate-200 font-semibold focus:outline-none focus:border-indigo-550"
//                 />
//               </div>

//               <div>
//                 <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Mobile Number (10 Digits)</label>
//                 <input
//                   type="text"
//                   required
//                   value={newCustPhone}
...
//                   className="w-full px-3 py-2 border border-slate-800 rounded-xl bg-slate-950 text-slate-200 font-semibold focus:outline-none focus:border-indigo-550 font-mono"
//                 />
//               </div>

//               <div>
//                 <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Complete Physical Address</label>
//                 <input
//                   type="text"
...
//               <div>
//                 <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Preferred Payment Mode</label>
//                 <select
...
//               <div>
//                 <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Internal Notes (Optional)</label>
//                 <textarea
//                   rows="2"
//                   value={newCustNotes}
//                   onChange={(e) => setNewCustNotes(e.target.value)}
//                   placeholder="Additional audit or delivery instructions..."
//                   className="w-full px-3 py-2 border border-slate-800 rounded-xl bg-slate-950 text-slate-200 font-semibold focus:outline-none focus:border-indigo-550"
//                 />
//               </div>
...
//       {/* PRINT STACK COMPILATION TEMPLATE REF */}
//       <div style={{ display: 'none' }}>
//         <div ref={printRef} style={{ width: '100%' }}>
//           <div style={{ textAlign: 'center', marginBottom: '20px' }}>
//             <h2 style={{ fontSize: '20px', fontWeight: 'bold', textTransform: 'uppercase' }}>{storeInfo.name}</h2>
...
//               <p><strong>PAYMENT:</strong> {paymentMethod} ({paymentMethod === 'Credit/Borrow' ? 'Credit' : 'Paid'})</p>
//             </div>
//           </div>
...
//                   <td style={{ padding: '8px 10px', textAlign: 'right' }}>₹{Number(item.price).toFixed(2)}</td>
//                   <td style={{ padding: '8px 10px', textAlign: 'right' }}>₹{Number(item.total).toFixed(2)}</td>
//                 </tr>
//               ))}
//             </tbody>
//           </table>
...
// // Sub-component for selected customer details preview card
// const selectedCustomerCard = (customerId, list) => {
//   const match = list.find(c => c.id === customerId);
//   if (!match) {
//     return (
//       <div className="bg-slate-900 border border-slate-850 p-3 rounded-xl text-[11px] text-slate-500 font-semibold italic text-center">
//         No borrow profile selected. Search above or click add.
//       </div>
//     );
//   }

//   return (
//     <div className="bg-slate-900 border border-slate-850 p-3 rounded-xl text-xs font-semibold text-slate-350 space-y-1">
//       <div className="flex justify-between items-center mb-1">
//         <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Active Profile</span>
//         <span className="text-[8px] bg-indigo-950 text-indigo-300 border border-indigo-900 px-1.5 py-0.5 rounded font-black uppercase tracking-wider">{match.customer_code}</span>
//       </div>
//       <div>Full Name: <span className="text-white font-bold">{match.name}</span></div>
//       <div>Mobile: <span className="text-slate-200 font-mono">{match.phone || '—'}</span></div>
//       <div>Payment Type: <span className="text-slate-200">{match.payment_mode || 'Cash'}</span></div>
//       {match.address && <div>Address: <span className="text-slate-400">{match.address}</span></div>}
//       {match.notes && <div>Notes: <span className="text-slate-450 italic">{match.notes}</span></div>}
//     </div>
//   );
// };
...
const SalesForm = ({ sale, onSubmit, onCancel }) => {
    ...
    const remainingBalance = Math.max(0, grandTotal - totalPaymentsEntered);
    const changeAmount = (totalPaymentsEntered > grandTotal && totalCashEntered > 0)
        ? Math.max(0, totalPaymentsEntered - grandTotal)
        : 0;
    ...
    return (
        ...
        <div className="flex flex-col h-[92vh] bg-white dark:bg-[#0b0d15] text-gray-800 dark:text-slate-100 font-sans select-none overflow-hidden">
            {/* ─── POS HEADER BAR (Dark Emerald) ─── */}
            <div className="bg-[#0F4C3A] dark:bg-[#0a1f18] px-6 py-3.5 flex flex-col sm:flex-row items-center justify-between border-b border-[#1a6b4f] dark:border-slate-800 flex-shrink-0 shadow-sm dark:shadow-none">
                <div className="flex items-center gap-3.5">
                    ...
                    <div>
                        <h1 className="text-lg font-bold tracking-tight text-white dark:text-slate-100">
                            {storeInfo.name}
                                    ...
                                    {filteredCustomers.length === 0 ? (
                                        ...
                                        filteredCustomers.map((c) => (
                                            ...
                                            >
                                                ...
                                                {customerType === 'Borrow' && Number(c.outstanding_balance || c.balance || 0) > 0 && (
                                                    <div className="flex items-center gap-1 text-[9px] font-bold text-red-500 dark:text-red-400">
                                                        <span>⚠ Bakaya:</span>
                                                        <span className="font-mono">₹{Number(c.outstanding_balance || c.balance || 0).toFixed(2)}</span>
                                                    </div>
                                                )}
                                            </div>
                                        ))
                ...
                <div className="lg:col-span-3 flex flex-col h-full overflow-hidden bg-white dark:bg-[#11141f] border border-gray-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm dark:shadow-xl dark:shadow-black/20">
                    <div className="flex-1 overflow-y-auto pr-1 space-y-4">
                        ...
                        <div className="space-y-2 text-xs font-medium text-gray-700 dark:text-slate-300">
                            <div className="flex justify-between bg-gray-50 dark:bg-[#0d101a] border border-gray-200 dark:border-slate-800 p-2.5 rounded-xl">
                                <span className="text-gray-500 dark:text-slate-400">Subtotal</span>
                                <span className="font-mono text-gray-800 dark:text-slate-200">₹{subtotal.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between bg-gray-50 dark:bg-[#0d101a] border border-gray-200 dark:border-slate-800 p-2.5 rounded-xl">
                                <span className="text-gray-500 dark:text-slate-400">Item Discount</span>
                                <span className="font-mono text-red-500 dark:text-red-400">-₹{totalItemDiscount.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between bg-gray-50 dark:bg-[#0d101a] border border-gray-200 dark:border-slate-800 p-2.5 rounded-xl">
                                <span className="text-gray-500 dark:text-slate-400">GST</span>
                                <span className="font-mono text-gray-800 dark:text-slate-200">₹{totalGst.toFixed(2)}</span>
                            </div>

                            <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/40 rounded-xl p-3 flex justify-between items-center text-sm font-bold text-emerald-700 dark:text-emerald-300 shadow-inner">
                                <span>Grand Total</span>
                                <span className="font-mono text-lg text-emerald-800 dark:text-white">₹{grandTotal.toFixed(2)}</span>
                            </div>

                            {/* ── Previous Balance (Bakaya) for Borrow Customers ── */}
                            {customerType === 'Borrow' && customerId && customerId !== 1 && (() => {
                                ...
                                return (
                                    <>
                                        <div className="flex justify-between bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40 p-2.5 rounded-xl">
                                            <span className="text-red-500 dark:text-red-400 font-semibold text-[11px]">⚠ Pichla Bakaya</span>
                                            <span className="font-mono text-red-600 dark:text-red-300 font-bold">₹{prevBal.toFixed(2)}</span>
                                        </div>
                                        <div className="bg-orange-50 dark:bg-orange-950/30 border border-orange-300 dark:border-orange-800/40 rounded-xl p-3 flex justify-between items-center text-sm font-bold text-orange-700 dark:text-orange-300 shadow-inner">
                                            <span className="text-[11px]">Total Outstanding</span>
                                            <span className="font-mono text-lg text-orange-800 dark:text-orange-200">₹{(grandTotal + prevBal).toFixed(2)}</span>
                                        </div>
                                    </>
                                );
                            })()}
                        ...
                        {/* Payment & Split Allocation Section (Redesigned for Crystal Clear Cash vs Digital Breakdown) */}
                        <div className="space-y-3" ref={paymentSectionRef}>
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-2 mb-1.5">
                                                <div className="flex items-center gap-1.5 flex-1">
                                                    ...
                                                    <select
                                                        ...
                                                        className="w-full px-2 py-1.5 text-xs font-black border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-[#121824] text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-[#0F4C3A]"
                                                    >
                                                        <option value="Cash">💵 Cash</option>
                                                ...
                                                {paymentRows.length > 1 && (
                                                    <button
                                                        ...
                                                        className="p-1.5 text-rose-500 hover:text-rose-700 dark:hover:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-950/60 rounded-lg transition-colors"
                                            ...
                                            {isCash && (
                                                <div className="flex items-center gap-1.5 pt-1">
                                                    <span className="text-[9px] font-bold text-slate-400 uppercase">Quick Cash:</span>
                                                    <button
                                                        type="button"
                                                        onClick={() => handlePaymentRowChange(row.id, 'amount', grandTotal)}
                                                        className="px-1.5 py-0.5 bg-white dark:bg-[#121824] border border-slate-300 dark:border-slate-700 rounded text-[9px] font-extrabold hover:bg-emerald-100 dark:hover:bg-emerald-950 text-slate-700 dark:text-slate-300"
                                                    >
                                                        Exact (₹{grandTotal})
                                                    </button>
                                                    {[100, 200, 500, 2000].map(val => (
                                                        <button
                                                            ...
                                                            className="px-1.5 py-0.5 bg-white dark:bg-[#121824] border border-slate-300 dark:border-slate-700 rounded text-[9px] font-bold hover:bg-emerald-100 dark:hover:bg-emerald-950 text-slate-600 dark:text-slate-400"
                                                        >
                                                            ₹{val}
                                                        </button>
                                                    ))}
                                            ...
                                            {/* Reference / Txn No for Non-Cash */}
                                            {!isCash && (
                                                <input
                                                    type="text"
                                                    value={row.referenceNo || ''}
                                                    onChange={(e) => handlePaymentRowChange(row.id, 'referenceNo', e.target.value)}
                                                    placeholder="Enter UTR / Approval / Txn Ref #"
                                                    className="w-full px-2 py-1 text-[10px] font-mono border border-slate-200 dark:border-slate-800 rounded-md bg-white dark:bg-[#121824] text-slate-700 dark:text-slate-300"
                                                />
                                            )}
                                        </div>
                                    );
                            ...
                            {/* VISUAL DASHBOARD SUMMARY: CLEAR CASH vs ONLINE BREAKDOWN */}
                            <div className="p-3 bg-slate-50 dark:bg-[#0d101a] border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
                                <div className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">Payment Mode Breakdown Summary</div>
                                
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-800 dark:text-emerald-300">
                                        <div className="text-[9px] font-bold uppercase text-emerald-600 dark:text-emerald-400">💵 Cash Paid</div>
                                        <div className="text-sm font-black font-mono">₹{totalCashEntered.toFixed(2)}</div>
                                    </div>
                                    <div className="p-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-800 dark:text-indigo-300">
                                        <div className="text-[9px] font-bold uppercase text-indigo-600 dark:text-indigo-400">💳 Online / Digital Paid</div>
                                        <div className="text-sm font-black font-mono">₹{totalNonCashEntered.toFixed(2)}</div>
                                    </div>
                                </div>

                                {changeAmount > 0 && (
                                    <div className="p-2.5 rounded-xl bg-emerald-500 text-white font-extrabold flex justify-between items-center shadow-md animate-pulse">
                                        <span className="text-xs uppercase tracking-wider">🪙 Change to Return:</span>
                                        <span className="font-mono text-base">₹{changeAmount.toFixed(2)}</span>
                                    </div>
                                )}

                                {remainingBalance > 0 && (
                                    <div className="p-2.5 rounded-xl bg-rose-500 text-white font-extrabold flex justify-between items-center shadow-md">
                                        <span className="text-xs uppercase tracking-wider">⚠️ Unpaid Balance:</span>
                                        <span className="font-mono text-base">₹{remainingBalance.toFixed(2)}</span>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="flex-shrink-0 pt-3 border-t border-gray-200 dark:border-slate-800 space-y-2 mt-2">
                        <button
                            onClick={handleSubmit}
                            type="button"
                            className="w-full flex items-center justify-center gap-2.5 py-3.5 bg-[#0F4C3A] hover:bg-[#1a6b4f] text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-sm shadow-emerald-200 dark:shadow-emerald-900/30 active:scale-[0.98] transition-all cursor-pointer"
                        >
                            <CheckCircleIcon className="w-4 h-4 stroke-[3]" />
                            Complete Sale <span className="text-[9px] font-mono opacity-60">(F8)</span>
                        </button>
                        <button
                            onClick={onCancel}
                            type="button"
                            className="w-full py-2.5 bg-gray-100 dark:bg-[#0d101a] hover:bg-gray-200 dark:hover:bg-slate-800/60 text-gray-600 dark:text-slate-400 hover:text-gray-800 dark:hover:text-white rounded-xl text-xs font-bold transition-all cursor-pointer border border-gray-200 dark:border-slate-800"
                        >
                            Cancel
                        </button>
                    </div>
...
const selectedCustomerCard = (customerId, list) => {
    ...
    return (
        <div className="bg-gray-50 dark:bg-[#0d101a] border border-gray-200 dark:border-slate-800 p-3 rounded-xl text-xs font-medium text-gray-700 dark:text-slate-300 space-y-1.5">
            ...
            {/* ── Previous Outstanding Balance (Bakaya) ── */}
            {match.customer_type === 'Borrow' && (
                <div className={`mt-1.5 flex justify-between items-center p-2 rounded-lg border ${
                    prevBalance > 0
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${
                        ...
                        {prevBalance > 0 ? '⚠ Pichla Bakaya' : '✓ No Dues'}
                    </span>
