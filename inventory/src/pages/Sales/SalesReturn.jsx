...
const SalesReturn = () => {
  ...
  const handleQtyChange = (productId, qty) => {
    const max = selectedItems[productId]?.maxQty || 0;
  ...
  const prevMax = (productId) => selectedItems[productId]?.maxQty || 0;

  // Calculate Total Refund Amount
  const totalCalculatedRefund = Object.keys(selectedItems).reduce((sum, pId) => {
  ...
  const handleProcessReturnSubmit = async (e) => {
    ...
    const checkedProductIds = Object.keys(selectedItems).filter(
      pId => selectedItems[pId].checked && selectedItems[pId].returnQty > 0 && selectedItems[pId].maxQty > 0
    );
    if (checkedProductIds.length === 0) {
      dispatch(showToast({ msg: 'Please select at least one eligible item with return quantity > 0', type: 'error' }));
      return;
    }

    const primaryProductId = Number(checkedProductIds[0]);
    const returnItem = selectedItems[primaryProductId];

    if (returnItem.returnQty > returnItem.maxQty || returnItem.returnQty <= 0) {
      dispatch(showToast({ msg: `Return quantity (${returnItem.returnQty}) must be between 1 and remaining sold quantity (${returnItem.maxQty})`, type: 'error' }));
      return;
    }

    setSubmitting(true);
    try {
      const returnItemsList = checkedProductIds.map(pId => ({
        product_id: Number(pId),
        quantity: selectedItems[pId].returnQty
      }));
                    ...
                    <table className="w-full text-left text-xs font-semibold">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                          ...
                          <th className="p-3">Eligible Qty</th>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                        {selectedInvoice.items && selectedInvoice.items.map((item) => {
                          const stateItem = selectedItems[item.product_id] || {};
                          const isChecked = stateItem.checked || false;
                          const returnQty = stateItem.returnQty || 1;
                          const lineTotal = item.unit_price * returnQty;
                          const isEligible = item.remaining_qty > 0;

                          return (
                            <tr key={item.product_id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-950/30 ${isChecked ? 'bg-emerald-50/20 dark:bg-emerald-950/20' : ''}`}>
                              <td className="p-3 text-center">
                                <input
                                  type="checkbox"
                                  disabled={!isEligible}
                                  checked={isChecked}
                                  onChange={(e) => handleItemCheck(item.product_id, e.target.checked)}
                              ...
                              <td className="p-3 font-black text-emerald-600 dark:text-emerald-400">{item.remaining_qty}</td>
                ...
                {loadingHistory ? (
                  ...
                  returnsHistory.map((ret) => (
                    <tr key={ret.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/30">
                      <td className="p-3 font-black text-emerald-600 dark:text-emerald-400">{ret.return_no}</td>
                      <td className="p-3 font-bold text-slate-800 dark:text-slate-200">{ret.invoice_no}</td>
                      <td className="p-3 font-bold text-slate-900 dark:text-white">
                        {ret.customer_name}
